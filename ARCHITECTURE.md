...
- `.t3/` is the single canonical state root
...
### 4.1 Single state root

All protocol-owned state lives under one root:

```text
.t3/
```

There is no shadow database, no alternate registry, and no duplicate state model in the npm layer.
...
Migration work happens in `.t3/sandbox/<timestamp>/worktree`, not on the operator's current branch.
...
flowchart LR
  ...
  CLI --> Store[".t3 canonical state root"]
...
flowchart LR
  ...
  Approve --> DiffRewrite["Rewrite .t3/patches/PATCH-###.diff"]
  Approve --> MetaRewrite["Rewrite .t3/patches/PATCH-###.meta.json"]
...
flowchart LR
  ...
  subgraph T3["T3 Tape"]
    Cmd["CLI commands"]
    ReadModel[".t3/triage.json and migration.log"]
...
- do not store credentials in `.t3/config.json`
