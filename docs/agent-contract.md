...
Provider selection happens from `.t3/config.json`:
...
Secrets rules:
- credentials are not written to `.t3/config.json`
- credentials are not written to `triage.json`
- credentials are not written to `.t3/sandbox/<timestamp>/resolved/*.json`
