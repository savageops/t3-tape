...
It solves one specific problem: a plain diff is not enough to keep a customization alive when upstream refactors the code around it. T3 Tape keeps the executable patch, the human-readable intent, and the operational metadata together as one PatchMD module inside `.t3`, so updates can be validated, triaged, re-applied, and approved without taking ownership of unrelated Theo-managed state.
...
- `t3-tape init` creates the PatchMD surfaces at `.t3/patch.md` plus `.t3/patch/`
...
3. Treat `.t3/patch/patches/*.diff` and `.t3/patch/patches/*.meta.json` as the machine-operational layer.
4. Treat `.t3/patch/triage.json` and `.t3/patch/migration.log` as the automation and audit surface.
5. Let agents help, but never let them become the source of truth.

## What T3 Tape Actually Owns

One T3 Tape-managed fork owns one root registry plus one plugin subtree:
...
| `.t3/patch/patches/*.diff` | executable diff layer | git, update engine |
| `.t3/patch/patches/*.meta.json` | operational metadata and confidence state | CLI, automation |
| `.t3/patch/triage.json` | latest update-cycle read model | bots, dashboards, CI |
| `.t3/patch/migration.log` | append-only audit trail | operators, audit tooling |
| `.t3/patch/config.json` | local PatchMD config | CLI, hooks, agent bridge |
| `.t3/patch/sandbox/` | ephemeral migration worktrees and staged artifacts | update flow only |
| `.t3/patch/state.lock` | exclusive writer lock for PatchMD-owned state | mutating commands only |

Rules that stay fixed:

- `.t3/` is the shared Theo namespace, not a PatchMD-owned monolith
- `.t3/patch.md` and `.t3/patch/**` are the only PatchMD-owned paths
- `.t3/patch/sandbox/` is ephemeral and should be gitignored
- `.t3/reports/` and any other foreign `.t3/*` siblings are tolerated and left untouched
...
2. records a started entry in `.t3/patch/migration.log`
3. fetches the requested upstream ref
4. creates `.t3/patch/sandbox/<timestamp>/`
5. creates a linked git worktree at `.t3/patch/sandbox/<timestamp>/worktree`
6. classifies each active patch as `CLEAN`, `CONFLICT`, or `MISSING-SURFACE`
7. writes `.t3/patch/triage.json` plus sandbox triage artifacts
...
The same summary is also persisted at `.t3/patch/triage.json`.
...
- rewrites `.t3/patch/patches/PATCH-001.diff`
- updates `.t3/patch/patches/PATCH-001.meta.json`, including diff-derived fields that must stay aligned with the approved diff
...
2. read JSON or files under `.t3/patch.md` and `.t3/patch/`
...
| nightly drift check | `t3-tape update --ref "$UPSTREAM_REF" --ci --confidence-threshold 0.90` | `.t3/patch/triage.json` |
| agent app or bot | `t3-tape triage --json` | JSON output |
| review UI or SaaS control plane | `t3-tape triage --json` | `.t3/patch/triage.json`, `.t3/patch/migration.log` |
...
| review dashboard | read `.t3/patch/triage.json` | render patch status, confidence, notes, unresolved assertions |
...
- review UIs backed by `.t3/patch/triage.json`
...
Hooks are configured in `.t3/patch/config.json`:
...
T3 Tape can call an external agent through `.t3/patch/config.json`.
...
- `T3_TAPE_AGENT_AUTH_TOKEN` is read from the environment for HTTP providers. Credentials are not stored in `.t3/patch/config.json`.
