...
PatchMD owns one root registry plus one plugin subtree inside Theo's shared `.t3` namespace:
...
Ownership rules:
- `.t3/patch.md` is the human-readable registry
- `.t3/patch/patches/*.diff` are the executable patch layer
- `.t3/patch/patches/*.meta.json` are the machine-operational layer
- `.t3/patch/triage.json` is the latest update-cycle read model
- `.t3/patch/migration.log` is append-only operational history
- `.t3/patch/config.json` is the operator-controlled protocol config
- `.t3/patch/state.lock` is an ephemeral exclusive lock file used to prevent concurrent PatchMD writers
...
1. code layer
   - unified diff stored at `.t3/patch/patches/PATCH-###.diff`
2. intent layer
...
Special tolerances:
- missing `.t3/patch/triage.json` before the first update cycle is allowed
...
- if staged code changes exist outside `.t3/patch.md` and `.t3/patch/`
