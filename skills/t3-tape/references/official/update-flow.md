...
- acquire an exclusive state lock at `.t3/patch/state.lock` (prevents concurrent PatchMD writers)
- append a `STARTED` entry to `.t3/patch/migration.log`
...
Actions:
- create `.t3/patch/sandbox/<sandbox-id>/`
- create a linked worktree at `.t3/patch/sandbox/<sandbox-id>/worktree`
...
- `.t3/patch/sandbox/<sandbox-id>/triage.json`
- `.t3/patch/triage.json`
- `TRIAGED` entry appended to `.t3/patch/migration.log`
...
```json
{
  ...
  "timestamp": "<utc-ts>",
    "sandbox": {
    "path": ".t3/patch/sandbox/<sandbox-id>",
    "worktree-branch": "t3-tape/migrate/<sandbox-id>",
    "worktree-path": ".t3/patch/sandbox/<sandbox-id>/worktree"
    },
  "patches": [
    {
      "id": "PATCH-001",
...
```text
.t3/patch/sandbox/<sandbox-id>/resolved/
  PATCH-001.diff
  PATCH-001.notes.txt
  PATCH-001.json
```
...
- stdout and stderr are captured under `.t3/patch/sandbox/<sandbox-id>/preview/`
...
- updates `.t3/patch/patches/PATCH-001.meta.json`, including diff-derived metadata such as `surface-hash`
...
`.t3/patch/migration.log` is append-only.
